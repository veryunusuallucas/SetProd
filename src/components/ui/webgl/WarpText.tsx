...
export interface Props {
  ...
  style?: CSSProperties;
  /**
   * Onde o texto encosta. O oficial so centraliza; numa tela com tudo alinhado
   * a esquerda, um titulo centralizado fica boiando fora da coluna.
   */
  align?: 'left' | 'center';
  /**
   * 0..1 — amplifica a distorcao. E por aqui que o easter egg acumula tensao.
   * Uma mola dentro do laco persegue este valor, entao clicar de novo no meio
...
const buildTextCanvas = ({ container, width, height, dpr, props }: BuildTextCanvasArgs): HTMLCanvasElement => {
  ...
  // drawLine centraliza em torno do x recebido. Para encostar a esquerda,
  // deslocamos o centro pela metade da largura da linha, com uma folga pequena
  // na borda para a distorcao nao cortar a primeira letra.
  const inset = width * 0.02;
  lines.forEach((line, index) => {
    const x = props.align === 'left'
      ? inset + measureLine(ctx, line, letterSpacing) / 2
      : width / 2;
...
const WarpText = ({
  ...
  useEffect(() => {
    ...
    const loop = (now: number): void => {
      ...
      // Mola critica perseguindo o boost pedido. Integrada aqui, e nao via
      // transicao de prop, para o acumulo ser continuo: clicar de novo no meio
      // do relaxamento soma, em vez de reiniciar a animacao.
      const dt = Math.min((now - lastFrame) * 0.001, 1 / 30);
      lastFrame = now;
      const alvoBoost = propsRef.current.boost;
      const acel = 120 * (alvoBoost - boost) - 2 * Math.sqrt(120) * boostVel;
      boostVel += acel * dt;
      boost += boostVel * dt;
      ...
      program.uniforms.uPointerStrength.value = base.pointerStrength * (1 + b * 4);
      program.uniforms.uRefraction.value = base.refraction * (1 + b * 6);
